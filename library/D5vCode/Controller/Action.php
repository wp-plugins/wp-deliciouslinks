 ...
 * $LastChangedBy$
 */
abstract class D5vCode_Controller_Action extends D5vCode_Base
{
	protected function marker($tag,$content)
	{
		$tagc = D5vCode_Tag::instance ();
		$matches =$tagc->get($tag, $content, true);
		foreach ((array) $matches as $match) {
			$new = call_user_func(array($this,$tag.'_Marker'),$match);
			$content = str_replace($match['match'], D5vCode_WP_Values::fixPostInsert($new), $content);
		}
		return $content;
	}
	private $_type = null;
	...
	protected function setView ()
	{
		if (null === $this->view)
		{
			$this->view = new D5vCode_View ( $this->application () );
		}
	}
	...
	public function loadTemplate ($filename)
	{
		...
		if($fullpath!==false)
		{
			...
			{
				case 'ini':
					$data = new D5vCode_Data_INI($this->application(),$fullpath);
					$return = $data->load();
					break;
				case 'csv':
					$data = new D5vCode_Data_CSV($this->application(),$fullpath,true);
					$return = $data->load();
					break;
				default:
