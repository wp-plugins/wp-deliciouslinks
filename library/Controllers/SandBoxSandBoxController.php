 ...
 * $LastChangedBy$
 */
class SandBoxSandBoxController extends D5vCode_Controller_Action_WP_AdminMenu
{
	public function FilesAction ( $content )
	{
		...
		$external = 0;
		$project = 0;
		$pclass = new D5vCode_Project ($this->application());
		$phpdoc = new D5vCode_Project ( $this->application () , $pclass->plugin () );
		$plugin = $phpdoc->blocks ();
		$plugin = $plugin [0];
		$phpdoc = new D5vCode_Project ( $this->application () , $pclass->readme () );
		...
		foreach ( $pclass->files () as $file )
		{
			...
			{
				$phpdoc = new D5vCode_Project ( $this->application () , $file ['file'] );
				$info = $phpdoc->blocks ();
				$error = array ();
				if (count ( $info ) > 0)
				{
	...
	protected function plugins ()
	{
		...
		{
			$projectsDir = new D5vCode_FS ( $this->application () , dirname ( dirname ( dirname ( dirname ( dirname ( __FILE__ ) ) ) ) ) . '/' . $dir . '/' );
			$php = $projectsDir->dir ( "*.php" , D5vCode_FS::type_file , 1 );
			...
			foreach ( $php as $file )
			{
				$p = new D5vCode_Application_Info ( $this->application () , $file );
				if ($p->author () == "DCoda Ltd")
				{
					$pmore ['info'] = $p;
					$pmore ['svn'] = new D5vCode_Application_Info_SVN ( $this->application () , $file );
					$pmore ['readme'] = new D5vCode_Application_info_Readme ( $this->application () , $file );
					$this->plugins [] = $pmore;
				}
			}
		}
	...
	{
		$mysql = new D5vCode_WP_Users ( );
	}

	//--- test
	public function testAction ()
	{
		$test = new D5vCode_Gzip ( $this->application () );
		$test->make_file ();
	}
}
