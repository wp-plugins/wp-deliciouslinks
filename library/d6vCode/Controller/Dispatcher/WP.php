 ...
 * $LastChangedBy$
 */
class d6vCode_Controller_Dispatcher_WP extends d6vCode_Controller_Dispatcher_Standard
{
	...
	public function controllers ()
	{
		...
		foreach ( $dirs as $dir )
		{
			$fs = new d6vCode_FS ( $this->application () , $dir );
	...
	public function setup ( $notmenu = false )
	{
		...
		foreach ( $this->controllers () as $controller )
		{
			...
			if ($notmenu)
			{
				...
				{
					case d6vCode_Controller_Action_WP_Abstract::WP_FILTER :
					case d6vCode_Controller_Action_WP_Abstract::WP_ACTION :
					case d6vCode_Controller_Action_WP_Abstract::WP_CONTROL :
						$controllerClass->setup ();
				}
			}
			else
			{
				if ($controllerClass->getType () == d6vCode_Controller_Action_WP_Abstract::WP_DASHBOARD)
				{
					$controllerClass->setup ();
				}
			}
