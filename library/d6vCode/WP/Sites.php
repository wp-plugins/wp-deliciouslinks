 ...
 * $LastChangedBy$
 */
class d6vCode_WP_Sites extends d6vCode_WP_Table
{
		...
		if (! $this->exists_by_domain ( $domain , $path ))
		{
			...
			if ($site_id)
			{
				...
				$options = array (
					...
					$prime_site_meta = new d6vCode_WP_SiteMeta ( 1 );
					$new_site_meta = new d6vCode_WP_SiteMeta ( $site_id );
	...
	{
		$wp_sitemeta = new d6vCode_WP_SiteMeta ( );
