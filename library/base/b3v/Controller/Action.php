<?php
abstract class b3v_Controller_Action extends b3v_Base {
	protected function marker($tag, $content) {
		$tagc = b3v_Tag::instance ();
		$matches = $tagc->get ( $tag, $content, true );
		foreach ( ( array ) $matches as $match ) {
			$new = call_user_func ( array ($this, $tag . '_Marker' ), $match );
			$content = str_replace ( $match ['match'], w3v_Values::fixPostInsert ( $new ), $content );
		}
		return $content;
	}
	private $_type = null;
	...
	protected function setView() {
		if (null === $this->view) {
			$this->view = new b3v_View ( $this->application () );
		}
	}
	...
	public function loadTemplate($filename) {
		...
		if ($fullpath !== false) {
			...
			switch (strtolower ( $pi ['extension'] )) {
				case 'ini' :
					$data = new b3v_Data_INI ( $this->application (), $fullpath );
					$return = $data->load ();
					$return = $return->getArray ();
					break;
				case 'csv' :
					$data = new b3v_Data_CSV ( $this->application (), $fullpath, true );
