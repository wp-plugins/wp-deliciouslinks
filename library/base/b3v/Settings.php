...
class b3v_Settings extends b3v_Controller_Action {
		...
		foreach ( $this->settings ['settings_folders'] as $folder ) {
			...
			while ( false !== ($entry = $d->read ()) ) {
				...
				if (fnmatch ( '*.phtml', $entry )) {
					...
					if (fnmatch ( '*_ext.phtml', $entry )) {
						$phtml_ext [$index] .= $this->renderScript ( $name, true );
					} else {
						$phtml [$index] .= $this->renderScript ( $name, true );
					}
				}
			}
			$d->close ();
