...
if (! class_exists ( 'b7v_Application' )) :
	...
	class b7v_Application extends b7v_Base {
		private $_page = null;
		...
		public function settings() {
			if (null == $this->settings) {
				$this->settings = new b7v_Settings ( $this );
			}
		...
		protected function set_frontcontroller($controller = null) {
			if (null === $controller) {
				$this->_frontcontroller = b7v_Controller_Front::getInstance ( $this->application () );
			} else {
		...
		protected function set_loader() {
			if (! class_exists ( 'b7v_Loader' )) {
				require_once dirname ( dirname ( __FILE__ ) ) . DIRECTORY_SEPARATOR . 'b7v/Loader.php';
			}
			$this->_loader = new b7v_Loader ( $this );
		...
		protected function preload_classes($classes = array()) {
			...
			$loader = $this->loader ();
			array_unshift ( $classes, 'b7v_Info', 'b7v_Controller_Action', 'b7v_Type_Abstract', 'b7v_Type_String', 'b7v_Type_Array', 'b7v_Debug', 'b7v_FS', 'b7v_View', 'b7v_Http', 'b7v_Tag', 'b7v_Data_Abstract', 'b7v_Data_INI', 'b7v_Data_CSV', 'b7v_Data_XML', 'b7v_FS', 'b7v_Http', 'b7v_Controller_Front', 'b7v_Controller_Dispatcher', 'b7v_Table', 'b7v_Controller_Action_Direct', 'b7v_Settings' );
