<?php
class w7v_Controller_Dispatcher extends b7v_Controller_Dispatcher {
	...
	public function controllers() {
		...
		foreach ( $dirs as $dir ) {
			$fs = new b7v_FS ( $this->application (), $dir );
		...
		foreach ( $this->controllers () as $controller ) {
			...
			if ($notmenu) {
				switch ($controllerClass->getType ()) {
					case w7v_Controller_Action_Abstract::WP_FILTER :
					case w7v_Controller_Action_Abstract::WP_ACTION :
					case w7v_Controller_Action_Abstract::WP_CONTROL :
						$controllerClass->setup ();
				}
			} else {
				if ($controllerClass->getType () == w7v_Controller_Action_Abstract::WP_DASHBOARD) {
					$controllerClass->setup ();
				}
			}
		}
