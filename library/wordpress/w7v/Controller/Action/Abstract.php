<?php
abstract class w7v_Controller_Action_Abstract extends b7v_Controller_Action {
	...
	protected function setView() {
		$this->view = new w7v_View ( $this->application () );
	}
	...
	protected function selected_action_wp() {
		...
		if (count ( $filter ) > 1 && $filter [1] == 'page') {
			...
			if (empty ( $pages ['page2'] )) {
				...
				foreach ( $this->actions () as $r ) {
					if ($pages ['page2'] == b7v_Type_String::staticSafe ( $r ['title'] )) {
						return $r;
					}
				}
			}
	...
	protected function subpages() {
		...
		foreach ( ( array ) $_GET as $key => $value ) {
			if (b7v_Type_String::staticStartsWith ( $key, 'page' )) {
				$pages [$key] = $value;
			}
		}
		ksort ( $pages );
