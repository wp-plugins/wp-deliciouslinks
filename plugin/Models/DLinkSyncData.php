<?php
/**
 * Model for the Redactor data
 * @package Redactor
 * @subpackage RedactorData
 * @copyright DCoda Ltd
 * @author DCoda Ltd
 ...
 * $LastChangedBy$
 */
class DLinkSyncData extends D5vCode_Base
{
	...
	{
		$values = $this->get();
		if ($values['between'] !='' && (time()-$values['last_updated'])/(60*60*24)>=$values['between'])
		//if ($values['between'] !='' && (time()-$values['last_updated'])/(1)>=$values['between'])
		{
	...
	public function set ( $values )
	{
		if ($this->global)
		{
			$data = new D5vCode_WP_SiteMeta ( );
		}
		else
		{
			$data = new D5vCode_WP_Options ( );
		}
	...
	{
		return array ( 
			'id' => '','password'=>'' , 'between' => '' , 'last_updated' => ''
		);
	}
	...
	{
		$delObj = new D5vCode_Http_Delicious();
		...
		$bookmarks = get_bookmarks ( array ( 
			'hide_invisible' => 0 , 'category_name' => 'Sync' 
		) );
